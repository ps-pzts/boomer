...
class TestSlippageMarketOrder:
    ...
    def test_market_buy_adds_slippage(self):
        ...
        result = sm.simulate_fill(
            ...
            order_price=500.0, bar_open=500.0, bar_high=510.0, bar_low=495.0,
        )
    ...
    def test_large_quantity_increases_slippage(self):
        ...
        small = sm.simulate_fill(
            OrderType.MARKET, OrderSide.BUY, 500.0, 500.0, 510.0, 495.0,
            quantity=10, avg_daily_volume=1_000_000,
        )
        large = sm.simulate_fill(
            OrderType.MARKET, OrderSide.BUY, 500.0, 500.0, 510.0, 495.0,
            quantity=10_000, avg_daily_volume=1_000_000,
        )
...
class TestSlippageLimitOrder:
    def test_limit_buy_fills_when_bar_low_crosses(self):
        sm = SlippageModel()
        result = sm.simulate_fill(
            OrderType.LIMIT, OrderSide.BUY, order_price=490.0,
            bar_open=500.0, bar_high=505.0, bar_low=488.0,
        )
        assert result.filled is True
    ...
    def test_limit_buy_does_not_fill_when_price_never_crosses(self):
        sm = SlippageModel()
        result = sm.simulate_fill(
            OrderType.LIMIT, OrderSide.BUY, order_price=480.0,
            bar_open=500.0, bar_high=505.0, bar_low=490.0,
        )
    ...
    def test_limit_sell_fills_when_bar_high_crosses(self):
        sm = SlippageModel()
        result = sm.simulate_fill(
            OrderType.LIMIT, OrderSide.SELL, order_price=510.0,
            bar_open=500.0, bar_high=515.0, bar_low=498.0,
        )
        assert result.filled is True
...
class TestSlippageStopOrder:
    def test_stop_sell_fills_when_bar_low_hits(self):
        sm = SlippageModel()
        result = sm.simulate_fill(
            OrderType.SL, OrderSide.SELL, order_price=490.0,
            bar_open=500.0, bar_high=505.0, bar_low=485.0,
        )
        assert result.filled is True
    ...
    def test_stop_sell_does_not_fill_when_low_above_stop(self):
        ...
        result = sm.simulate_fill(
            OrderType.SL, OrderSide.SELL, order_price=490.0,
            bar_open=500.0, bar_high=505.0, bar_low=492.0,
        )
        assert result.filled is False

    def test_stop_slippage_factor_is_1_5x_base(self):
        sm = SlippageModel()
        market = sm.simulate_fill(
            OrderType.MARKET, OrderSide.BUY, 500.0, 500.0, 510.0, 495.0
        )
        stop = sm.simulate_fill(
            OrderType.SL, OrderSide.BUY, order_price=500.0,
            bar_open=500.0, bar_high=505.0, bar_low=498.0,
        )
